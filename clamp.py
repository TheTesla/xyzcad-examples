...
def f(x,y,z):
    ...
    if y > hf:
        return False
    ang = +math.atan2((z-zf1),x-xf1)
    r = 2*screwprofile((1.5*y+ang+math.pi)%(2*math.pi)) + ((x-xf1)**2 + (z-zf1)**2)**0.5
    if r < rg + 0.1:
        return False
    ang = +math.atan2((z-zf2),x-xf2)
    r = 2*screwprofile((1.5*y+ang+math.pi)%(2*math.pi)) + ((x-xf2)**2 + (z-zf2)**2)**0.5
    if r < rg + 0.1:
        return False
    ...
    if (rf**2 > (z-zf1)**2 + (x-xf1)**2):
        return True
    if (rf**2 > (z-zf2)**2 + (x-xf2)**2):
        return True
    if (rf**2 > (z-zf12)**2 + (x-xf12)**2):
        return True
